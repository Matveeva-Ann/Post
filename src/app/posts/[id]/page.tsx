      ...
      <div className="flex items-center justify-center">
        {isSuccess && !isError && (
          <div className="w-full bg-cover bg-no-repeat flex flex-col items-center justify-center">
            <Link href={'/'} className="mb-2 flex items-center gap-3 cursor-pointer p-3 text-lg">
              <FaLongArrowAltLeft />
              Go Back
            </Link>

            <div className="bg-white dark:bg-black dark:text-white rounded-md text-black p-4 z-10 w-6/12 relative">
              <span className="text-sm text-gray-600 absolute top-2 right-4">{formattedDateTime(data.createdAt)}</span>
              <Image
